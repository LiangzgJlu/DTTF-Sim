...
自动驾驶车辆的配置信息按照Protobuf进行定义。
```protobuf
syntax = "proto3";
package dttf.rpc;

message RpcHeader{
  string version = 1;      # DTTF-Sim 版本
  uint64 timestamp = 2;    # 消息时间戳
}
...
message CameraConfig{                   # 相机传感器配置
  string name = 1;                      # 相机名
  Location location = 2;                # 相机相对于车辆中心的偏移
  Rotation rotation = 3;                # 相机姿态
  uint32 fov = 4;                       # 相机FOV
  uint32 width = 5;                     # 像宽
  uint32 height = 6;                    # 像高 
  double lens_flare_intensity = 7;      # 镜头光晕强度
  Communication communication = 8;      # 相机传感器数据通信方式

}

message LidarConfig{                    # 激光雷达传感器配置
  string name = 1;                      # 激光雷达名
  Location location = 2;                # 激光雷达相对于车辆中心的偏移
  Rotation rotation = 3;                # 激光雷达的姿态
  uint32 channels = 4;                  # 激光雷达的通道数
  uint32 range = 5;                     # 激光雷达的探测半径
  uint32 rotation_frequency = 6;        # 激光雷达的旋转频率
  uint32 horizontal_fov = 7;            # 激光雷达的水平视野范围
  int32 upper_fov = 8;                  # 激光雷达的垂直-上极限视野
  int32 lower_fov = 9;                  # 激光雷达的垂直-下极限视野

  Communication communication = 11;     # 激光雷达传感器数据通信方式
}

message GnssConfig{                     # GNSS传感器配置
  string name = 1;                      # 设备名
  Location location = 2;                # 设备相对于车辆中心安装位置
  Rotation rotation = 3;                # 设备姿态
  Communication communication = 11;     # 传感器通信方式
}

message ImuConfig{                      # IMU传感器配置
  string name = 1;                      # 设备名
  Location location = 2;                # 设备相对于车辆中的偏移
  Rotation rotation = 3;                # 设备姿态
  Communication communication = 11;     # 设备通信方式
}
...
message RadarConfig{                    # Radar传感器配置
  ...
  Location location = 2;                # 设备相对于车辆中心安装位置
  Rotation rotation = 3;                # 设备姿态
  uint32 horizontal_fov = 4;            # 毫米波雷达水平视野范围
  uint32 range = 5;                     # 毫米波雷达探测距离
  uint32 vertical_fov = 6;              # 毫米波雷达垂直视野范围
  Communication communication = 11;     # 毫米波雷达数据通信方式
}

message GroundTrueConfig{               # 真值传感器配置
  string name = 1;                      # 真值传感器名
  uint32 range = 2;                     # 真值传感器探测范围
  Communication communication = 11;     # 真值传感器通信方式
}

message LocalizationConfig{             # 定位传感器配置 (GNSS+IMU)
  string name = 1;                      # 定位传感器名
  Location location = 2;                # 定位传感器相对于车辆中心的安装位置
  Rotation rotation = 3;                # 定位传感器姿态
  Communication communication = 11;     # 定位传感器数据通信方式
}

message ChassisConfig{                  # 底盘配置
  string name = 1;                      # 底盘名
  Communication communication = 11;     # 底盘数据通信方式
}

message ControlConfig{                  # 车辆控制信息配置
    string name = 1;                    # 控制信息名
    Communication communication = 11;   # 车辆控制信息通信方式
}

message AutonomousVehicleConfigCommand{ # 自动驾驶车辆配置命令
  string name = 1;                      # 自动驾驶车辆名
  string type = 2;                      # 自动驾驶车辆类型，参照Carla支持的车辆类型(如: vehicle.tesla.model3)

  Location location = 3;                # 初始时车辆位置信息(x: 经度， y: 纬度， z: 高程)
  Rotation rotation = 4;                # 初始时车辆姿态信息
  ...
  ChassisConfig chassis = 13;           # 底盘配置
  ControlConfig control = 14;           # 控制配置
}
